...
# Copyright (C) 2019 Luan Halaiko (tecnotailsplays@gmail.com)
...
if [ ! -d "$TOOL_DIR" ]; then
  mkdir $TOOL_DIR
  git clone https://aosp.tuna.tsinghua.edu.cn/platform/prebuilts/gcc/linux-x86/aarch64/aarch64-linux-android-4.9 ../toolchains/aarch64-linux-android-4.9
fi   
...
makekornel()
{
  ...
  echo -e "$cyan#######################################################################$nc"
  echo -e "$purple(i) Total time: $(($DIFF / 60)) minute(s) and $(($DIFF % 60)) seconds.$nc"
  echo -e "$cyan#######################################################################$nc"
}
