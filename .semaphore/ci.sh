...
function push_package() {
	JIP="Nite-Kernel-$ZIP_VERSION-$BUILD_TYPE-$BUILD_POINT.zip"
...
cp $IMG nito-ak2/
cd nito-ak2/
zip -r9 -9 "Nite-Kernel-$ZIP_VERSION-$BUILD_TYPE-$BUILD_POINT.zip" .
md5sum Nite-Kernel-$ZIP_VERSION-$BUILD_TYPE-$BUILD_POINT.zip >> "md5sum_$(git log --pretty=format:'%h' -1).txt"
